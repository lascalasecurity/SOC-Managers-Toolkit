...
function isRetriablePurpleError(objOrText) {
  const t = typeof objOrText === 'string' ? objOrText : JSON.stringify(objOrText);
  return /\b500\b|Internal Server Error|\/sdl\/v2\/api\/queries|502|503|504|timeout|timed out/i.test(t);
}
...
export async function runPowerQuery({ cwd, query, startMs, endMs, startIso, endIso, retries = 4, timeoutMs = 120000 }) {
  const args = { query };
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    const r = await mcporterCall({ cwd, tool: 'purple-mcp.powerquery', args, output: 'json', timeoutMs });

    try {
      const obj = JSON.parse(r.stdout);
      // Normalize mcporter-wrapped errors (see lib/mcporter.js)
      if (obj && typeof obj === 'object' && obj.error) {
        last = { error: obj.error, attempt };
        if (attempt < retries && isRetriablePurpleError(obj.error)) {
          const backoff = Math.min(60000, 1500 * Math.pow(2, attempt));
          await sleep(backoff);
          continue;
        }
        return { error: obj.error, attempt, retried: attempt > 0 };
      }
      return obj;
    } catch {
      last = { error: r.stdout || 'powerquery: unparseable response', stderr: r.stderr, attempt };
      if (attempt < retries && isRetriablePurpleError(r.stdout || r.stderr || '')) {
        const backoff = Math.min(60000, 1500 * Math.pow(2, attempt));
        await sleep(backoff);
        continue;
      }
      return { ...last, retried: attempt > 0 };
    }
  }
