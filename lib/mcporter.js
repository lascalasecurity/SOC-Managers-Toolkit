...
export async function mcporterCall({ tool, args = {}, cwd, env = {}, output = 'json', timeoutMs = 120000 }) {
  ...
  // NOTE: When isError=true, mcporter often returns this JS object literal even when --output json.
  // We convert that into a small JSON payload so downstream callers can reliably detect errors.
  if (stripped.includes('content:') && stripped.includes("type: 'text'") && stripped.includes('text:')) {
    try {
      const isError = stripped.includes('isError: true');
      ...
      while ((m = re.exec(afterText))) {
        ...
        // Stop once we have a plausible payload; usually the first chunk is the full text.
        if (chunks.length >= 1) break;
      }
      if (chunks.length) {
        const dt = chunks.join('').trim();
        if (isError) {
          // Preserve the full error text in a deterministic JSON wrapper.
          return { stdout: JSON.stringify({ error: dt }), stderr };
        }
